*** Settings ***
Documentation     Tests for cart functionality on the demo blaze website with logged in user.
...
Login User
    ...
    ${promise}=    Promise To    Wait For Response    matcher=**/login    timeout=3s
    Click    //div[@class='modal fade show']//button[@class='btn btn-primary' and text()='Log in']
    ${body}=    Wait For    ${promise}
    Should Be Equal    ${body.status}    ${200}
